...
@AndroidEntryPoint
class WebAudioCallingActivity : BaseActivity<ChatViewModel>() {
...
//    private var callReceive: Boolean? = null
    ...
    override fun setupUi() {
        ...
        ActivityCompat.requestPermissions(this@WebAudioCallingActivity, permissions(), 1)

        binding.videocallUsername.text = remoteUser

        Glide.with(this@WebAudioCallingActivity)
    ...
    private fun requestCameraAndMicAccess() {
        if (IntentUtil.cameraPermission(this@WebAudioCallingActivity)
            && IntentUtil.readAudioPermission(this@WebAudioCallingActivity)
            && IntentUtil.readVideoPermission(this@WebAudioCallingActivity)
            && IntentUtil.readImagesPermission(this@WebAudioCallingActivity))
        {
            init()
        } else{
    ...
    private fun onCameraPermission() {
        ...
        Dexter.withContext(this@WebAudioCallingActivity)
            .withPermissions(
                Manifest.permission.READ_MEDIA_IMAGES,
                Manifest.permission.READ_MEDIA_VIDEO,
                Manifest.permission.READ_MEDIA_AUDIO,
